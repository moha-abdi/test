...
async def get_messages(client, message):
    ...
    if message.chat.id == copyfrom[0]:
        ...
        gp = [s['symbol'] for s in exchange_info['symbols'] if re.match(
            f'(?i){s["symbol"]}|#{s["symbol"]}|{s["baseAsset"]}/{s["quoteAsset"]}|#{s["baseAsset"]}/{s["quoteAsset"]}',
            message.text)]
        try:
            pair = gp[0]
            if pair in file:
...
def quantitycal(symbol, investment):
    ...
    if symbol == 'BTCUSDT':
        ...
        price = float(cli.get_symbol_ticker(symbol=symbol)['price'])
        precision = (cli.get_symbol_info(symbol=symbol))
        qtty = round(investment / price, rround(lotsize))
        info = cli.futures_exchange_info()
        prc = {si['symbol']: si['quantityPrecision'] for si in info['symbols'] if si['symbol'] == symbol}
        s = prc[f"{symbol}"]
...
qttyy = quantitycal(symbol='ETHUSDT', investment=config.investment)
print("QAUNTITY", qttyy)
# symbol = 'ANKRUSDT'
# info = cli.futures_exchange_info()
# print("TASTAAA", sss)
# print("waataaan", s)
